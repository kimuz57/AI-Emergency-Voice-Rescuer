...
const APP_URL = process.env.NEXT_PUBLIC_APP_URL || "https://kws.wattanapong.com";
...
function QRGeneratorTab() {
  ...
  const formatMAC = (value: string) => {
    // Auto-format MAC address: เอาตัวไม่ใช่ hex ออก แล้วใส่ : ทุก 2 ตัว
  ...
  const generateQR = useCallback(async () => {
    ...
    setError("");
    try {
      const qrUrl = `${APP_URL}/devices?mac=${macInput}`;
        ...
        <div className="flex justify-end mt-5">
          ...
          >
            {isGenerating ? (
              ...
              <>
                <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}><path strokeLinecap="round" strokeLinejoin="round" d="M12 4v1m6 11h2m-6 0h-2v4m0-11v3m0 0h.01M12 12h4.01M16 20h4M4 12h4m12 3.5V16M4.5 4.5h3v3h-3v-3zm9 0h3v3h-3v-3zm-9 9h3v3h-3v-3z" /></svg>
                สร้าง QR Code
              </>
            )}
          </button>
      ...
      {qrDataUrl && (
        <div className="dark:bg-slate-700/50 bg-white/60 backdrop-blur-md p-6 md:p-8 rounded-2xl shadow-sm border border-white/80 flex flex-col items-center gap-6 animate-in fade-in slide-in-from-bottom-4 duration-500">
          ...
          <div className="text-center">
            ...
            {deviceLabel && (
              <p className="text-sm text-slate-500 dark:text-slate-400 mt-1">{deviceLabel}</p>
            )}
            <p className="text-xs text-slate-400 mt-2 break-all max-w-xs">
              🔗 {APP_URL}/devices?mac={macInput}
            </p>
          </div>
...
// ============================================================
// 📋 TAB 2: Patient Registration Form Component
// ============================================================
function RegisterPatientTab({ scannedMAC }: { scannedMAC: string }) {
  ...
  // Sync boardId ถ้า scannedMAC เปลี่ยน
  useEffect(() => {
    if (scannedMAC) {
      setFormData((prev) => ({ ...prev, boardId: scannedMAC }));
    }
  }, [scannedMAC]);

  const handleChange = (e: React.ChangeEvent<HTMLInputElement | HTMLSelectElement>) => {
    setFormData({ ...formData, [e.target.name]: e.target.value });
    if (e.target.name === "caregiverEmail") setCaregiverError("");
  };
    ...
    try {
      ...
      if (res.ok) {
        ...
        setFormData({
          ...
          medicalCondition: "",
          caregiverEmail: "",
          boardId: scannedMAC, // คง MAC ไว้ถ้าสแกนมา
          deviceName: "ไมค์หัวเตียง",
        });
      } else {
        if (data.field === "caregiverEmail") {
          setCaregiverError(data.error);
        } else if (data.field === "boardId") {
          alert("❌ " + data.error);
        } else {
    ...
    <form onSubmit={handleSubmit} className="space-y-6">
      ...
      <div className="dark:bg-slate-700/50 bg-white/60 backdrop-blur-md p-6 md:p-8 rounded-2xl shadow-sm border border-white/80 flex flex-col gap-5 relative overflow-hidden">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
          {/* ชื่อ-นามสกุล */}
          <div>
            <label className="dark:text-slate-300 block text-xs font-bold text-slate-600 mb-1 ml-1 uppercase tracking-wide">ชื่อ-นามสกุล</label>
          ...
          <div className="grid grid-cols-2 gap-4">
            <div>
              <label className="dark:text-slate-300 block text-xs font-bold text-slate-600 mb-1 ml-1 uppercase tracking-wide">อายุ (ปี)</label>
              <input type="number" name="age" value={formData.age} onChange={handleChange}
                className="dark:bg-slate-800 dark:text-white w-full px-4 py-3 rounded-xl bg-slate-50/50 border border-slate-200 focus:border-purple-500 focus:ring-2 focus:ring-purple-200 outline-none transition-all text-sm text-slate-700" />
            </div>
            <div>
              <label className="dark:text-slate-300 block text-xs font-bold text-slate-600 mb-1 ml-1 uppercase tracking-wide">เพศ</label>
          ...
          {/* หมายเลขห้อง */}
          <div>
            <label className="dark:text-slate-300 block text-xs font-bold text-slate-600 mb-1 ml-1 uppercase tracking-wide">หมายเลขห้อง</label>
            <input type="text" name="roomNumber" value={formData.roomNumber} onChange={handleChange} required placeholder="เช่น 101A"
              className="dark:bg-slate-800 dark:text-white w-full px-4 py-3 rounded-xl bg-slate-50/50 border border-slate-200 focus:border-purple-500 focus:ring-2 focus:ring-purple-200 outline-none transition-all text-sm text-slate-700" />
          </div>

          {/* โรคประจำตัว */}
          <div>
            <label className="dark:text-slate-300 block text-xs font-bold text-slate-600 mb-1 ml-1 uppercase tracking-wide">โรคประจำตัว (ถ้ามี)</label>
            <input type="text" name="medicalCondition" value={formData.medicalCondition} onChange={handleChange} placeholder="เช่น ความดัน, เบาหวาน"
              className="dark:bg-slate-800 dark:text-white w-full px-4 py-3 rounded-xl bg-slate-50/50 border border-slate-200 focus:border-purple-500 focus:ring-2 focus:ring-purple-200 outline-none transition-all text-sm text-slate-700" />
          </div>

          {/* อีเมลผู้ดูแล */}
          <div className="dark:bg-slate-800/40 md:col-span-2 bg-purple-50/50 p-4 rounded-xl border border-purple-100">
            <label className="dark:text-purple-400 block text-xs font-bold text-purple-800 mb-1 ml-1 uppercase tracking-wide">
              อีเมลผู้ดูแล (Caregiver Email) <span className="text-red-500">*</span>
            </label>
            <input required type="email" name="caregiverEmail" value={formData.caregiverEmail} onChange={handleChange}
              ...
              className={`dark:bg-slate-800 dark:text-white w-full px-4 py-3 rounded-xl bg-white border outline-none transition-all text-sm text-slate-700 ${
                caregiverError ? "border-red-400 focus:ring-2 focus:ring-red-200" : "border-purple-200 focus:border-purple-500 focus:ring-2 focus:ring-purple-200"
              }`} />
            {caregiverError && (
            ...
            <p className="dark:text-purple-400 text-xs text-purple-600/70 mt-2 ml-1">ระบบจะส่งการแจ้งเตือนฉุกเฉินไปยังผู้ดูแลที่ระบุไว้</p>
          </div>
        </div>
      </div>
      ...
      <div className="dark:bg-slate-700/50 bg-white/60 backdrop-blur-md p-6 md:p-8 rounded-2xl shadow-sm border border-white/80 flex flex-col gap-5 relative overflow-hidden">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
          ...
          <div>
            ...
            {scannedMAC ? (
              /* ถ้าสแกนมา — แสดงเป็น Badge ล็อคค่า */
              <div className="relative">
                <div className="dark:bg-emerald-900/30 w-full px-4 py-3 rounded-xl bg-emerald-50 border-2 border-emerald-400 flex items-center justify-between">
                  <span className="font-mono text-sm font-bold text-emerald-700 dark:text-emerald-400 tracking-widest uppercase">
                    {scannedMAC}
                  </span>
                  <span className="flex items-center gap-1 text-xs font-bold text-emerald-600 dark:text-emerald-400 bg-emerald-100 dark:bg-emerald-900/50 px-2 py-1 rounded-lg">
                ...
                <p className="text-emerald-600 dark:text-emerald-400 text-xs mt-1.5 ml-1 flex items-center gap-1">
                  ...
                  MAC Address ถูกดึงมาจาก QR Code อัตโนมัติ
                </p>
              </div>
            ) : (
              /* ถ้าไม่มี — กรอกเองตามปกติ */
              <input
                type="text"
                name="boardId"
          ...
          {/* จุดติดตั้ง */}
          <div>
            <label className="dark:text-slate-300 block text-xs font-bold text-slate-600 mb-1 ml-1 uppercase tracking-wide">จุดติดตั้งอุปกรณ์</label>
            <input type="text" name="deviceName" value={formData.deviceName} onChange={handleChange} required
...
function DevicesPageContent() {
  ...
  // ถ้ามี ?mac= → เปิด Tab ลงทะเบียนอัตโนมัติ, ถ้าไม่มี → เปิด Tab สร้าง QR
  ...
  return (
    <div className="relative min-h-screen flex items-center justify-center p-4 md:p-8 font-sans overflow-hidden">
      {/* Background Glowing Orbs Removed */}
      ...
      <div className="dark:bg-slate-800 relative z-10 w-full max-w-4xl bg-white/80 backdrop-blur-xl rounded-3xl shadow-2xl p-6 md:p-10 border border-white/60">
        ...
        <div className="text-center mb-8">
          <div className="dark:bg-slate-700 inline-flex items-center justify-center w-16 h-16 rounded-2xl bg-gradient-to-br from-blue-100 to-purple-100 mb-4 shadow-sm">
            <svg xmlns="http://www.w3.org/2000/svg" className="w-8 h-8 text-blue-600 dark:text-blue-400" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
              ...
              <path strokeLinecap="round" strokeLinejoin="round" d="M12 11h4M12 16h4" />
        ...
        <div>
          ...
          {activeTab === "register" && <RegisterPatientTab scannedMAC={scannedMAC} />}
        </div>
      </div>
    </div>
  );
...
// ============================================================
// Export default — ห่อด้วย Suspense เพราะใช้ useSearchParams
// ============================================================
