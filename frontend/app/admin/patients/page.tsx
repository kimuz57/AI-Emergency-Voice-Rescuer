...
export default function AdminPatients() {
    ...
    const checkAdminAndFetchData = async () => {
      try {
        ...
        if (Array.isArray(usersData)) {
          const caregiversOnly = usersData.filter(
            (u: any) => u.role === "caregiver" || u.role === "admin",
          );
  ...
  const openEditPopup = (patient: any) => {
    ...
    const existingCaregiverIds = patient.Caregivers
      ? patient.Caregivers.map((c: any) => c.ID || c.id)
      : [];
  ...
  const handleUpdatePatient = async () => {
    ...
    try {
      const res = await fetch(
        `${API_URL}/api/admin/patients/${editPatient.ID}`,
        {
          method: "PUT",
          credentials: "include",
  ...
  const handleCaregiverToggle = (caregiverId: number) => {
    setEditForm((prev) => {
      ...
      if (isSelected) {
        return {
          ...prev,
          caregiver_ids: prev.caregiver_ids.filter((id) => id !== caregiverId),
        };
      } else {
  ...
  const filteredCaregivers = availableCaregivers.filter(
    (cg) =>
      (cg.name &&
        cg.name.toLowerCase().includes(caregiverSearch.toLowerCase())) ||
      (cg.email &&
            ...
            <div className="dark:bg-slate-800 bg-white rounded-2xl shadow-2xl w-full max-w-md mx-4 p-6 overflow-y-auto max-h-[90vh]">
                    ...
                    <div className="max-h-32 overflow-y-auto p-2 flex flex-col gap-1">
                      {filteredCaregivers.length > 0 ? (
                        filteredCaregivers.map((cg) => {
                          const isChecked = editForm.caregiver_ids.includes(
                            cg.id || cg.ID,
                          );
                          return (
                            <label
                              key={cg.id || cg.ID}
                              className="flex items-center gap-3 cursor-pointer p-1.5 hover:bg-white dark:hover:bg-slate-600 rounded transition-colors"
                            >
                              <input
                                type="checkbox"
                                checked={isChecked}
              ...
              <div className="flex gap-3">
                ...
                <button
                  ...
                  className="flex-1 px-4 py-2.5 bg-indigo-600 hover:bg-indigo-700 text-white rounded-xl text-sm font-semibold transition-colors shadow-sm"
                >
                  บันทึกข้อมูล
                </button>
              </div>
